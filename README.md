...
> 当前状态：跨语言 v1 契约、安全项目服务、内容寻址 Artifact Store 与可重建 SQLite 索引已建立；产品界面与工作流执行能力仍在后续里程碑中。
...
| `crates/narracut-core` | Cargo package | 不依赖 UI 的安全项目服务、Artifact Store、SQLite 索引与迁移逻辑 |
