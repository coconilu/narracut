...
flowchart LR
  ...
  Runtime --> Receipt["完整 enqueue receipt\n原子幂等身份"]
...
`enqueue_script_stage` 在读取当前凭据或 Research 状态之前，先把完整、已通过 Provider v1 Schema
校验的 enqueue 请求原子写入 `requests/jobs/<jobId>.json`。`jobId` 仍由 `projectId +
idempotencyKey` 确定性推导；文件采用无覆盖提交，因此并发调用只有一份请求能取得该身份。
...
| 升级前 legacy Job 已存在 | 写 receipt 前先按旧 hash 校验 Job，并由不可变快照、executor 与配置重建完整请求；仅 exact replay 可附加 receipt |
...
与重试策略。receipt 文件存在性与内容哈希另行校验，不参与算法选择。缺少版本字段的 legacy Job 永远按
旧 hash 读取，错误旁路 receipt 不会污染 `get/list/recover`；不同语言、token、Provider 或模型也不能借升级
重放抢占 receipt。
