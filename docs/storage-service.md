...
`PRAGMA user_version = 1`。连接启用外键、5 秒 busy timeout、WAL 与 `synchronous=NORMAL`。
...
| `job_summaries` | PR05 任务队列使用的当前状态摘要 | 后续项目内 JobEvent / 任务恢复流程 |
...
| `list_indexed_jobs` | 查询 PR05 将维护的任务摘要 |
...
| 操作 | 当前上限 | 后续接管者 |
| --- | ---: | --- |
| 单文件 Artifact 导入或完整哈希校验 | 64 MiB | PR05 持久化任务队列提供进度、取消与恢复 |
| 项目 Artifact 重建 | 4096 条 / 元数据 64 MiB / 来源边 65,536 条 | PR05 后台扫描任务 |
| cache 清理 | 4096 条目 / 256 MiB / 64 层 | PR05 后台维护任务 |
